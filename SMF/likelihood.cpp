...
// CALCULATE LIKELIHOOD (negative of actual log-likelihood)
	double likeli = 0.5*(num_pts*log(sigma2) + num_pts + log(det+detpad));

// ADD PENALTY FOR LARGE THETA :
	double k_penalty = 0.5E0*pow(num_pts,0.5);
	double th_sum = 0.0;
	for( int i=0; i<dim_th ; i++)
		th_sum = th_sum + theta[i]*theta[i];
// Penalty applied on the 2-norm of theta
	likeli = likeli + 0.5*k_penalty*log(th_sum); 

...
void poll_mads(VecD x, int n, VecI spc, VecD amin,VecD amax, int &n_poll, MatD &poll_pts, bool *flag)
{
...
//	std::srand(std::time(NULL));
    ...
    // Compute minimum grid size for each dimension
    for(i=0;i<n;i++)
    {
	...
	// --- Step 0: ensure base row is inside domain
	...
	//loop counter to prevent getting stuck
	int ct = 0;
	while(!indom)
	{
		...
		max_dsize = pow(2,n_poll); 
		dir = 2*(randn(2)) - 1; 
	
		base_val = dir*max_dsize ; 
		...
		for(i=0; i<n ;i++)
		{
			dx[i] = base_row[i]*interv[i]; 
			xbase[i] = x[i] + dx[i]; 
		}

		indom = bndchk(n,xbase,amin,amax);

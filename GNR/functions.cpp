...
int run_comp_test(vessel& curr_vessel, double P_low, double P_high, double lambda_z_test) {
    int sn = curr_vessel.sn;

    double lambda_z_store = curr_vessel.lambda_z_curr;
    double P_store = curr_vessel.P;
    double a_low = curr_vessel.a_mid[sn];

    int equil_check = 0;
    int num_P = 10; 
    double comp [num_P];

    double P_incr = (P_high - P_low) / (num_P * 1.0);
...
/*         double norm_p = (curr_vessel.P - P_store)/P_store;
        double norm_r = (curr_vessel.a_mid[sn] - a_mid_store)/a_mid_store;
        if (norm_p != 0) {
            val += pow((norm_r / norm_p),2); 
        } */
        curr_vessel.P += P_incr;
    }
    double a_high = curr_vessel.a_mid[sn];

    double out = ((a_high-a_low)/a_low)/((P_high-P_low)/P_low);
    curr_vessel.Exp_out << out << "\t" << "\n";
    curr_vessel.lambda_z_curr = lambda_z_store;
    curr_vessel.P = P_store;
    equil_check = find_iv_geom(&curr_vessel);
    return 0;

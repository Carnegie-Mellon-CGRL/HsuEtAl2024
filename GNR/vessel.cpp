...
void vessel::initializeTEVG(string scaffold_name, string immune_name, vessel const &native_vessel, double n_days_inp, double dt_inp) {
    ...
    double Ki_drhopdt = 1.00 * Ki_rhop; //K_i_p_wound; //
    ...
    if (abs(dQdt_p1 * epsilonR_alpha_0[0]) > abs(dQdt_p2 * epsilonR_alpha_0[1])){
        dQdt_p2 = abs( dQpdt(zeta_p1 * gamma_p_d2, k_p2 * gamma_p_d1, zeta_p2 * gamma_p_d2) );
    }
    else{
        dQdt_p1 = abs( dQpdt(zeta_p2 * gamma_p_d2, k_p1 * gamma_p_d1, zeta_p1 * gamma_p_d2) );
    }
    max_deg_rate = (dQdt_p1 * epsilonR_alpha_0[0] + dQdt_p2 * epsilonR_alpha_0[1]) / (epsilonR_alpha_0[0] + epsilonR_alpha_0[1]);

    // printf("%s %f\n", "max_deg_rate: ", 1 / max_deg_rate);
    // fflush(stdout);
    ...
    for (int sn = 1; sn < nts; sn++) {
        ...
        // printf("%s %f %s %f\n", "Time: ", s, "ups_drhoRpdt: ", ups_drhoRpdt);
        // fflush(stdout);
